...
public abstract class AbstractDoubleLinkDistributedIndex extends AbstractSingleLinkDistributedIndex{
	...
	protected abstract InetSocketAddress getPrevMachine();
	...
	public LoadDataBox checkLoad(LoadInfoTable loadInfoTable, MessageSender sender) {
		...
		if(this.getPrevMachine() != null && loadInfoTable != null && loadInfoTable.getLoadList() != null
			...
			ldb.setPrevLoad(loadInfoTable.getLoadList().get(this.getPrevMachineIPString()));
			ldb.setPrevDataSize(loadInfoTable.getDataSizeList().get(this.getPrevMachineIPString()));
		}
	...
	public boolean moveData(LoadDataBox ldb){
		...
		synchronized (this) {
			...
			if(ldb.getNextLoad()!=-1
					&&(ldb.getMyLoad()> ldb.getPrevDataSize())
					){
